...
int main() {
	...
	// 黑將//紅先走
	if(moveXiangqiRecord(record, 9, 4, 8, 4)==-1) printf("紅先走\n");
    else {
		printf("紅先走.error\n");
		return -1;
	}
	// 紅兵//兩步
	if(moveXiangqiRecord(record, 3, 4, 5, 4)==-1) printf("兵兩步\n");
	else {
		printf("兵兩步.error\n");
		return -1;
	}
	// 紅馬//一步
	if(moveXiangqiRecord(record, 0, 7, 1, 7)==-1) printf("紅馬一步\n");
    else {
		printf("紅馬一步.error\n");
		return -1;
	}
	// 紅象//兩步
	if(moveXiangqiRecord(record, 0, 6, 2, 6)==-1) printf("紅象兩步\n");
    else {
		printf("紅象兩步.error\n");
		return -1;
	}
    ...
    // 紅不用炮//王自殺
    if(moveXiangqiRecord(record, 3, 4, 4, 4)==-1) printf("紅不用炮擋車，王自殺\n");
    else {
		printf("紅不用炮擋車，王自殺.error\n");
		return -1;
	}
	// 紅炮
    ...
    // 紅馬//王自殺
    if(moveXiangqiRecord(record, 4, 7, 2,6)==-1) printf("紅馬王自殺\n");
    else {
		printf("紅馬王自殺.error\n");
		return -1;
	}
